...
func main() {
	...
	resourceFlags := append([]cli.Flag{
		...
		&cli.StringFlag{
			...
			Aliases:     []string{"p"},
			Usage:       `The pattern of the resource name. The pattern supports an incremental index via '*' (same semantic as Go's os.CreateTemp()) and a set of placeholders expanded per resource: {type}, {rp}, {name}, {root_scope}. E.g. '{type}' may expand to 'virtual_machines'. (only works for multi-resource mode).`,
			Value:       "res-",
	...
	resourceGroupFlags := append([]cli.Flag{
		&cli.StringFlag{
			...
			EnvVars:     []string{"AZTFEXPORT_NAME_PATTERN"},
			Aliases:     []string{"p"},
			Usage:       `The pattern of the resource name. The pattern supports an incremental index via '*' (same semantic as Go's os.CreateTemp()) and a set of placeholders expanded per resource: {type}, {rp}, {name}, {root_scope}. E.g. '{type}*' may expand to 'virtual_machines0'.`,
			Value:       "res-",
			Destination: &flagset.flagPattern,
		},
	}, commonFlags...)
